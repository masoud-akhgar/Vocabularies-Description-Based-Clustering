...
class Some_Widgets(GUI):  # inherits from the GUI class
    def __init__(self, parent, controller):
        ...
        frame1 = tk.LabelFrame(self, frame_styles, text="Output")
        frame1.place(rely=0.05, relx=0.02, height=400, width=400)

        frame2 = tk.LabelFrame(self, frame_styles, text="Widgets")
        frame2.place(rely=0.05, relx=0.45, height=600, width=500)

        button1 = tk.Button(frame2, text="تایید", command=lambda: Refresh_data())
        button1.pack()

        Var1 = tk.IntVar()
        Var2 = tk.IntVar()
        Var3 = tk.IntVar()
        # Cbutton1 = tk.Checkbutton(frame2, text="اعمال فیلتر بر حسب زمان", variable=Var1, onvalue=1, offvalue=0)
        # Cbutton1.pack()
        # Cbutton2 = tk.Checkbutton(frame2, text="tk CheckButton2", variable=Var2, onvalue=1, offvalue=0)
        # Cbutton2.pack()

        Cbutton3 = ttk.Checkbutton(frame2, text="اعمال فیلتر بر حسب زمان", variable=Var1, onvalue=1, offvalue=0)
        Cbutton3.pack()
        Cbutton5 = ttk.Checkbutton(frame2, text="نمایش نمودار", variable=Var2, onvalue=1, offvalue=0)
        Cbutton5.pack()
        Cbutton4 = ttk.Checkbutton(frame2, text="نمایش نام شهر بجای آیدی", variable=Var3, onvalue=1, offvalue=0)
        Cbutton4.pack()
        label1 = tk.Label(frame2, font=("Verdana", 12), text="عنوان جستجو")
        label1.pack()
        inputtxt1 = tk.Text(frame2,height = 1,width = 20)
        inputtxt1.pack()
        label2 = tk.Label(frame2, font=("Verdana", 12), text=" ex. 2022 سال میلادی (تاریخ شروع)")
        label2.pack()
        inputtxt2 = tk.Text(frame2,height = 1,width = 20)
        inputtxt2.pack()
        label3 = tk.Label(frame2, font=("Verdana", 12), text="ماه میلادی (تاریخ شروع)  ex. 12")
        label3.pack()
        inputtxt3 = tk.Text(frame2,height = 1,width = 20)
        inputtxt3.pack()
        label4 = tk.Label(frame2, font=("Verdana", 12), text="روز میلادی (تاریخ شروع) ex. 15")
        label4.pack()
        inputtxt4 = tk.Text(frame2,height = 1,width = 20)
        inputtxt4.pack()
        label5 = tk.Label(frame2, font=("Verdana", 12), text="سال میلادی (تاریخ پایان)")
        label5.pack()
        inputtxt5 = tk.Text(frame2,height = 1,width = 20)
        inputtxt5.pack()
        label6 = tk.Label(frame2, font=("Verdana", 12), text="ماه میلادی (تاریخ پایان)")
        label6.pack()
        inputtxt6 = tk.Text(frame2,height = 1,width = 20)
        inputtxt6.pack()
        label7 = tk.Label(frame2, font=("Verdana", 12), text="روز میلادی (تاریخ پایان)")
        label7.pack()
        inputtxt7 = tk.Text(frame2,height = 1,width = 20)
        inputtxt7.pack()
        ...
        Var4 = tk.IntVar()
        Var5 = tk.IntVar()
        R1 = tk.Radiobutton(frame2, text="displayNumberOfOccuranceInTime(یک کلمه یا خالی)", variable=Var5, value=1)
        R1.pack()
        R2 = tk.Radiobutton(frame2, text="displayNumberOfOccuranceInCities", variable=Var5, value=2)
        R2.pack()
        R3 = tk.Radiobutton(frame2, text="displayNumberOfSearchedTermInACity", variable=Var5, value=3)
        R3.pack()
        R4 = tk.Radiobutton(frame2, text="similarWords", variable=Var5, value=4)
        R4.pack()
        ...
        def Refresh_data():
            ...
            if Var5.get():
                Project().displayNumberOfOccuranceInTime(label1.cget("text"),(int(label4),int(label3),int(label2)),
                (int(label7),int(label6),int(label5)),Var1.get() ,Var2.get())
